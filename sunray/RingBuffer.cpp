...
ERingBuffer::ERingBuffer(unsigned int size)
{
	_size = size;
...
void ERingBuffer::init()
{
	ringBufP = ringBuf;
	memset(ringBuf, 0, _size+1);
}

void ERingBuffer::push(char c)
{
	*ringBufP = c;
	ringBufP++;
...
bool ERingBuffer::endsWith(const char* str)
{
...
void ERingBuffer::getStr(char * destination, unsigned int skipChars)
{
...
void ERingBuffer::getStrN(char * destination, unsigned int skipChars, unsigned int num)
{
	int len = ringBufP-ringBuf-skipChars;
